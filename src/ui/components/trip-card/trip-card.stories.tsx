    ...
    () => (
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 400px)', gap: '24px', padding: '24px' }}>
        <div>
          ...
          <TripCard trip={randomTrip('all-variants-polaroid')} variant={TripCardVariant.Polaroid} />
        </div>
        <div>
          <h3 style={{ marginBottom: '8px', fontSize: '14px', fontWeight: 600 }}>Immersive</h3>
          <TripCard trip={randomTrip('all-variants-immersive')} variant={TripCardVariant.Immersive} />
        </div>
      </div>
    ),
  ],
