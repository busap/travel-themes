...
export function AuroraTheme({ trip, config }: AuroraThemeProps) {
  ...
  const renderHeader = () => (
    <div className={styles.header}>
      <h1 className={`${styles.title} ${titleClasses} ${playfair.className}`}>
        {trip.name}
      </h1>
      <p className={`${styles.subtitle} ${bodyClasses} ${crimson.className}`}>
  ...
  const renderPhotoSections = () => {
    ...
    const getPhotoClass = (width: 'narrow' | 'medium' | 'wide') => {
      const variantClass = width === 'narrow' ? styles.photoNarrow :
          width === 'medium' ? styles.photoMedium : styles.photoWide;
