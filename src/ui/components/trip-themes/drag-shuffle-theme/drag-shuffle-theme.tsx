...
export function DragShuffleTheme({ trip, config }: DragShuffleThemeProps) {
  ...
  const handlePointerMove = (event: PointerEvent<HTMLDivElement>) => {
    if (!isPointerDragging || isAnimating || !pointerStartRef.current) return;
  ...
  const renderTripMeta = () => (
    <header className={styles.tripPanel}>
      ...
      <h1 className={`${styles.tripName} ${titleClasses}`}>{trip.name}</h1>
      <p className={`${styles.tripDescription} ${bodyClasses}`}>
        {trip.countries.join(', ')}
        {trip.year ? ` • ${trip.year}` : ''}
      </p>
      {trip.description && <p className={styles.tripSummary}>{trip.description}</p>}
    </header>
  );
  ...
  const renderStackCard = (
    ...
    const placeName =
      photo.title?.trim() || `${trip.countries[0]} stop ${activeIndex + stackIndex + 1}`;
    ...
    return (
      <article
        ...
        onPointerCancel={isTopCard ? () => clearGestureState() : undefined}
  ...
  return (
    <section
      ...
      style={{ '--swipe-duration': `${animationDurationMs}ms` } as CSSProperties}
    >
      <div className={styles.layout}>
        ...
        <div className={styles.deckPanel}>
          ...
          >
            {hasCards ? (
              visibleCards.map((photo, index) => renderStackCard(photo, index))
            ) : (
              <div className={styles.emptyState}>No photos available</div>
            )}
          </div>
          ...
          {hasCards && (
            <div className={styles.progressRow}>
              <button
                ...
                disabled={!canRevert}
                style={isFirstItem ? {visibility: "hidden"} : undefined}
              ...
              <p className={styles.progress}>
                {(activeIndex % deckPhotos.length) + 1} / {deckPhotos.length}
              </p>
            </div>
          )}
        </div>
