...
export function getPolaroidScale(index: number): number {
  const scales = [1, 0.98, 1.03, 0.96, 1.02, 0.97, 1.05, 0.99, 1.01, 0.95, 1.04];
  return scales[index % scales.length];
}

export function getPolaroidOffset(index: number): { x: number; y: number } {
...
export function getPolaroidRotationMobile(index: number): number {
  const rotations = [-2, 1, -3, 2, -1, 3, -2.5, 1.5, -1.5, 2.5];
  return rotations[index % rotations.length];
}
