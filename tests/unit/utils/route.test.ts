...
describe('route utilities', () => {
  describe('buildRoute', () => {
    ...
    it('should build trip detail route with id parameter', () => {
      const result = buildRoute(RouteKey.TripDetail, { id: 'japan-2023' });
      expect(result).toBe('/trip/japan-2023');
    });

    it('should URL-encode route parameters', () => {
      const result = buildRoute(RouteKey.TripDetail, { id: 'trip with spaces' });
      expect(result).toBe('/trip/trip%20with%20spaces');
    });

    it('should handle special characters in parameters', () => {
      const result = buildRoute(RouteKey.TripDetail, { id: 'trip-2024/special' });
    ...
    it('should throw error when parameter is undefined', () => {
      ...
      expect(() => buildRoute(RouteKey.TripDetail, { id: undefined })).toThrow(
        'Missing required parameter "id" for route "tripDetail"'
      );
    });
  ...
  describe('getTripRoute', () => {
    ...
    it('should handle different trip IDs', () => {
      expect(getTripRoute('morocco-markets')).toBe('/trip/morocco-markets');
